...
- APP 鉴权过滤器：`AppTokenAuthenticationFilter`（默认拦截 `/app/**（兼容 /api/app/**）`）
...
3. APP 业务接口建议统一使用 `/app/**（兼容 /api/app/**）` 路径前缀。
