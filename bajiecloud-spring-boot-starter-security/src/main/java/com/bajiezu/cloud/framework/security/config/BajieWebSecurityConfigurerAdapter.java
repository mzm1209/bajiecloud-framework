...
public class BajieWebSecurityConfigurerAdapter {
     ...
     * |   如果有参数，参数表示IP地址，如果用户IP和参数匹配，则可以访问 hasRole             |   如果有参数，参数表示角色，则其角色可以访问
     * permitAll           |   用户可以任意访问 rememberMe          |   允许通过remember-me登录的用户访问 authenticated
     * |   用户登录后可访问
     */
    @Bean
    protected SecurityFilterChain filterChain(HttpSecurity httpSecurity) throws Exception {
        ...
        httpSecurity
                ...
                .authorizeHttpRequests(c -> c
                        ...
                        // 添加 Swagger UI 资源放行
                        .requestMatchers(SecurityFrameworkUtils.NOT_NEED_LOGIN_PATHS.toArray(new String[0])).permitAll()
        ...
        // 添加 Token Filter
        authenticationTokenFilter.setPermitAllPaths(permitAllUrls.values().stream().toList());
