...
import static com.bajiezu.cloud.common.util.collection.CollectionUtils.convertList;
...
public class BajieWebSecurityConfigurerAdapter {
  ...
  protected SecurityFilterChain filterChain(HttpSecurity httpSecurity) throws Exception {
    ...
    Set<String> noNeedLoginPaths = SecurityFrameworkUtils.rewriteNotNeedLoginPaths(contextPath);
    ...
    httpSecurity
        ...
        .authorizeHttpRequests(c -> c
                ...
                // 1. 公开Actuator健康检查端点（给负载均衡器或K8s探针使用）
//            .requestMatchers("/**/actuator/health", "/**/actuator/health/**").permitAll()
                // 2. 其他Actuator端点可要求特定角色（如ADMIN）或IP白名单
    ...
    for (Map.Entry<RequestMappingInfo, HandlerMethod> entry : handlerMethodMap.entrySet()) {
      ...
      if (entry.getKey() != null) {
        if (entry.getKey().getPatternsCondition() != null &&
            CollectionUtil.isNotEmpty(entry.getKey().getPatternsCondition().getPatterns())) {
          urls.addAll(entry.getKey().getPatternsCondition().getPatterns());
        }
        if (entry.getKey().getPathPatternsCondition() != null &&
            CollectionUtil.isNotEmpty(entry.getKey().getPathPatternsCondition().getPatterns())) {
          urls.addAll(
              convertList(entry.getKey().getPathPatternsCondition().getPatterns(),
      ...
      urls.forEach(url -> urls.add(contextPath + url));
