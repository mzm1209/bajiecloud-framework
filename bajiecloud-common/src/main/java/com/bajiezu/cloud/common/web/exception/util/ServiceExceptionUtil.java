package com.bajiezu.cloud.common.web.exception.util;
...
public class ServiceExceptionUtil {
  ...
  public static String doFormat(int code, String messagePattern, Object... params) {
    ...
    for (l = 0; l < params.length; l++) {
      ...
      if (j == -1) {
        log.error("[doFormat][参数过多：错误码({})|错误内容({})|参数({})", code, messagePattern,
            params);
        if (i == 0) {
    ...
    if (messagePattern.indexOf("{}", i) != -1) {
      log.error("[doFormat][参数过少：错误码({})|错误内容({})|参数({})", code, messagePattern,
          params);
    }
    stringBuilder.append(messagePattern.substring(i));
    return stringBuilder.toString();
