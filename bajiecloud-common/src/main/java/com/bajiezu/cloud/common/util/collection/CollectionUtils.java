package com.bajiezu.cloud.common.util.collection;
...
public class CollectionUtils {
  ...
  public static boolean containsAny(Object source, Object... targets) {
    return Set.of(targets).contains(source);
  }
  ...
  public static <T, K> Map<K, T> convertMapByFilter(Collection<T> from, Predicate<T> filter,
  ...
  public static <T, U> U findFirst(Collection<T> from, Predicate<T> predicate,
      Function<T, U> func) {
    if (CollUtil.isEmpty(from)) {
      return null;
    }
  ...
  public static <T, V extends Comparable<? super V>> V getSumValue(Collection<T> from,
    ...
    assert !from.isEmpty(); // 断言，避免告警
    return from.stream().map(valueFunc).filter(Objects::nonNull).reduce(accumulator)
        .orElse(defaultValue);
  }
  ...
  public static <T> List<T> newArrayList(List<List<T>> list) {
    return list.stream().filter(Objects::nonNull).flatMap(Collection::stream)
        .collect(Collectors.toList());
  }
